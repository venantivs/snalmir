 ...
 * TODO: Atomizar init_http_server(). Resolver problema de buffer overflow.
 */
...
void* init_http_server()
{
	...
	while (true)
	{
		...
		if (!strcasecmp(request_info.method, "POST") || !strcasecmp(request_info.method, "HEAD") || !strcasecmp(request_info.method, "PUT") || !strcasecmp(request_info.method, "DELETE") || !strcasecmp(request_info.method, "CONNECT") || !strcasecmp(request_info.method, "OPTIONS") || !strcasecmp(request_info.method, "TRACE") || !strcasecmp(request_info.method, "PATCH"))
		{
...
static bool get_request(int comm_fd, struct request* request_info)
{
	...
	if (ioctl(comm_fd, FIONREAD, &available_bytes) == -1)
		fatal_error("ioctl");
	...
	char *host_pos = strcasestr(buffer, "host"); 
